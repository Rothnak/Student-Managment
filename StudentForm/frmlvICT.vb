...
Public Class frmlvICT
    ...
    Private Sub frmlvICT_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        ConnectDB()
        Try
            MysqlConn.Open()
            Dim Query As String

            Query = "SELECT st.student_id,st.fname,st.lname,st.Gender,st.DOB,st.Phone,st.Address,cur.course_name,sh.shift_name,st.Years,st.Payment,st.Image FROM tbl_student as st INNER JOIN tbl_course as cur ON st.course=cur.course_id INNER JOIN tbl_shift as sh ON sh.shift_id=st.shift WHERE course_id = 1"
